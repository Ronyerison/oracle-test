...
try {
	...
	function useskill_capt_onthefly(obj) {
		...
		function runCapt() {
			...
			var TIME_TO_SUBMIT = obj.timetosubmit ? obj.timetosubmit : 20;
			var TIME_TO_SUBMIT_INITIAL = obj.timetosubmitinitial ? obj.timetosubmitinitial : 10;
			...
			var TIME_SUBMIT = 1000 * TIME_TO_SUBMIT, 
				TIME_TO_SUBMIT_INITIAL = 1000 * TIME_TO_SUBMIT_INITIAL,
				ultimaAcao = null, sending = false, LOCALSTORAGE_VAR = "";
				...
				if (DEBUG) {
					if (CONNECTED_PLUGIN) {
						...
						if (typeof stack == 'string') {
							stack = 'DEBUG:' + stack.substr(5);
						}
						console.log(arguments, new Date(), stack);
					}
				}
			}
			...
			function Action(action, time, url, content, tag, tagIndex, id,
				...
				this.sTime = time;
				this.sUrl = url;
				this.sContent = String(content).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/\t/g, ' ').replace(/\r?\n|\r/g, '').trim();				
			...
			if (isMobile()) {
				...
				document.addEventListener("mousemove", function(e) {
					if ( (e.timeStamp - lastMouseMove >= 1500) && (e.timeStamp - lastMouseMove <= 5000) ) {
			...
			function getAcoesString(isGarbage) {
				return getItemLocalStorage(isGarbage);
			}

			function getAcoes(isGarbage) {
				var str = getAcoesString(isGarbage);
			...
			function addAcao(acao) {
				if (CONNECTED_PLUGIN) {
					...
					debug("Add Action: ", acao, arr);
					setItemLocalStorage(arr);
				}
			}

			function setItemLocalStorage(acoes, isGarbage) {
				var x = stringfyJSON(acoes);
				var name = isGarbage ? LOCALSTORAGE_VAR + "_garbage" : LOCALSTORAGE_VAR;
			...
			function getItemLocalStorage(isGarbage) {
				var name = isGarbage ? LOCALSTORAGE_VAR + "_garbage" : LOCALSTORAGE_VAR;
			...
			function getVisibleElements() {
				var all = document.getElementsByTagName("*"), elems = [];
			...
			function getOracleVisibleElements() {
				return getVisibleElements().length;
			}
